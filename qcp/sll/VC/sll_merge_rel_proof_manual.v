...
Proof.
  ...
  + subst.
    ...
    rewrite (whilebreak_unfold _ _) in H4.
    prove_by_one_abs_step (inl (l1_new, y_data :: l2_new, l3_2 ++ x_data :: nil)).
    unfold merge_body.
    abs_choice_left.
    abs_test_step; [ lia | ].
    abs_return_step.
...
Proof.
  ...
  + subst.
    ...
    unfold merge_from_mid_rel in *.
    rewrite (whilebreak_unfold _ _) in H4.
    prove_by_one_abs_step (inl (x_data :: l1_new, l2_new, l3_2 ++ y_data :: nil)).
    unfold merge_body.
    abs_choice_right.
    abs_test_step; [ lia | ].
    abs_return_step.
...
Proof.
  ...
  rewrite (whilebreak_unfold _ _) in H1.
  prove_by_one_abs_step (inr (l3 ++ l1)).
  unfold merge_body.
  destruct l1; abs_return_step.
Qed.
...
Proof.
  ...
  prove_by_one_abs_step (inr (l3 ++ l2)).
  unfold merge_body.
  abs_return_step.
Qed.
...
Proof.
  ...
  Intros retl retaddr.
  Exists retl retaddr.
  eapply safeExec_return_Atrue_finnal in H0 as [? ? ].
  entailer!.
  eapply gmergesortrec_incr;eauto.
  eapply gmergesortrec_perm;eauto.
Qed.
...
Proof.
  ...
  Intros retl retaddr.
  Exists retl retaddr.
  eapply safeExec_return_Atrue_finnal in H as [? ? ].
  entailer!.
  eapply gmergesortrec_incr;eauto.
  eapply gmergesortrec_perm;eauto. 
Qed.
...
Proof.
  ...
  eapply safeExec_return_Atrue_finnal in H as [? ? ].
  entailer!.
  eapply mergesortrec_incr;eauto.
  eapply mergesortrec_perm;eauto. 
Qed.
